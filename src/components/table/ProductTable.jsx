...
import { COLUMNS } from '../../common/table-columns/product'
	...
	const handleDeleteProduct = () => {
		switch (actionType) {
			case ActionTypeEnum.DELETE_ONE:
				deleteProduct([currentRowAction.original.id])
				break
	...
	return (
		<Box>
			<Flex justifyContent='space-between' mb='5'>
				...
				<ReachLink to='/product/add' style={{ display: 'flex' }}>
					<Center
						bg='purple.400'
						borderRadius='3xl'
						pl='3'
									...
									<Td>
										<Center justifyContent='left'>
											<Box
												...
												mr='7'
												onClick={() =>
													navigate(`/product/edit/${row.original.id}`)
												}
											>
												<AiOutlineEdit />
											</Box>
				...
				<ModalContent>
					...
					<ModalBody>
						{actionType === ActionTypeEnum.DELETE_ONE &&
							`Bạn có chắc muốn xoá sản phẩm ${currentRowAction.original.name}`}

						{actionType === ActionTypeEnum.DELETE_MANY &&
							`Bạn có chắc muốn xoá ${selectedFlatRows.length} sản phẩm`}
					</ModalBody>
